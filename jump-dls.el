;;; jump-dls.el --- Jump to definition of symbol using various methods.
...
(global-set-key (kbd "M-.") 'jump-symbol-at-point)
(global-set-key (kbd "C-M-.") 'jump-back)

(require 'imenu)
(require 'etags)

(defvar jump-build-index nil
  "Whether jumping for imenu and tags will construct the index if necessary.")
...
additional jump methods."
  (interactive "P")
  (condition-case nil
      (jump-symbol (or (jump-get-symbol-at-point)
                       (error "No symbol found at point")))
    (error (jump-symbol (intern (read-input (format "No jump method found for '%s'. Jump to:" (jump-get-symbol-at-point))))))))
...
(defun jump-imenu-search-buffer (str)
  ...
  (let (jump-imenu--seen-it
        pos)
    (when jump-build-index (imenu-rescan))
